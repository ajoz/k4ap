...
/**
 * An activity representing a list of Features. This activity
 * has different presentations for handset and tablet-size devices. On
 * handsets, the activity presents a list of items, which when touched,
 * lead to a [FeatureDetailActivity] representing
...
class FeatureListActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val recyclerView = findViewById(R.id.feature_list)!!
        setupRecyclerView(recyclerView as RecyclerView)

        if (findViewById(R.id.feature_detail_container) != null) {
        ...
        override fun onBindViewHolder(holder: ViewHolder, position: Int) {
            ...
            holder.mView.setOnClickListener { v ->
                if (mTwoPane) {
                    ...
                    arguments.putString(FeatureDetailFragment.ARG_ITEM_ID, holder.mItem!!.id)
                    val fragment = FeatureDetailFragment()
                    fragment.arguments = arguments
                    supportFragmentManager.beginTransaction()
                            .replace(R.id.feature_detail_container, fragment)
                            .commit()
                } else {
                    val context = v.context
                    val intent = Intent(context, FeatureDetailActivity::class.java)
                    intent.putExtra(FeatureDetailFragment.ARG_ITEM_ID, holder.mItem!!.id)
